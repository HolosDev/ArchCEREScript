...
instance (TextShow vp, TextShow (vi v)) => TextShow (VariablePosition vp vi v) where
  showb (VP vPlace vIndex) =
    showb vPlace <> TS.singleton '[' <> showb vIndex <> TS.singleton ']'


instance (TextShow vp, TextShow (vi v), TextShow vt, TextShow co, TextShow (eis vp vi v vt co)) => Show (ArchCERES vp vi v vt co eis) where
  show = toString . showb

instance (TextShow vp, TextShow (vi v), TextShow vt, TextShow co, TextShow (eis vp vi v vt co)) => TextShow (ArchCERES vp vi v vt co eis) where
  ...
  showb (CRSReplaceTextBy vpA vpB vpC ) = showbCS3 "ReplaceTextBy" vpA vpB vpC
  showb (CRSReplaceTextByTo vpA vpB vpC vpD) =
    showbCS4 "ReplaceTextByTo" vpA vpB vpC vpD
  showb (CRSGetPointer vpA vpB  ) = showbCS2 "GetPointer" vpA vpB
  showb (CRSSetPointer vpA vpB  ) = showbCS2 "SetPointer" vpA vpB
  ...
  showb (CRSTo0 cH                ) = showbCSC0 "To0" cH
  showb (CRSTo1 cH vpA            ) = showbCSC1 "To1" cH vpA
  showb (CRSTo2 cH vpA vpB        ) = showbCSC2 "To2" cH vpA vpB
  showb (CRSTo3 cH vpA vpB vpC    ) = showbCSC3 "To3" cH vpA vpB vpC
  showb (CRSTo4 cH vpA vpB vpC vpD) = showbCSC4 "To4" cH vpA vpB vpC vpD
